...
public class Board {
    ...
    private final int NORMAL_TIMER = 15, SOFT_TIMER = 5; // Timers for dropping pentominoes
    ...
    public void update() {
        ...
        if(numRows > 0){
            ...
            for(i = 0; i<numRows; i++){
                ...
                // Drop everything above the row (NAIVE GRAVITY)
                /*for (int row = targetRows[i]; row>=1; row--) {
                    for (int column=0; column<field[targetRows[i]].length; column++) {
                ...
                for (int row=0; row<field.length; row++) {
                    for (int column=0; column<field[row].length; column++) {
                        if (field[row][column] != null) {
                            field[row][column].xx = column;
                            field[row][column].yy = row;
                        }
                    }
                }*/
            }
     ...
     */
     ...
     public void createNewPentomino() {
        ...
        if(batch.size() == 0){
            createBatch();
        if(batch.size() == 1 || batch.size() == 0){
            if(batch.size()==1){
                int param;
                param = batch.get(0);
                batch.remove(0);
                createBatch();
                batch.set(param,batch.get(0));
            }
            else createBatch();
            }
        }
    ...
    //Creates the batch of 12 pentominos
    ...
    // Returns nextPent for interface
    public int nextPent(){
        if(batch.size() < 1){
            createBatch();
        }
